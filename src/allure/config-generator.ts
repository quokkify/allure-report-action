...
function normalizeModuleTokens(value: string): string[] {
  return String(value || '')
    .normalize('NFKD')
    .toLowerCase()
    .replace(/[\u0300-\u036f]/g, '')
...
function parseVariableParts(key: string): VariableParts | null {
...
function tokensEndWith(longer: string[], shorter: string[]): boolean {
...
function generateEnvironmentId(value: string, used: Set<string>): string {
  const base =
    String(value || '')
      .normalize('NFKD')
      .toLowerCase()
...
function readModuleVariables(resultsDir: string): Record<string, string> {
  ...
  return document as Record<string, string>;
}
...
export async function generateModuleConfig(options: ModuleConfigOptions): Promise<void> {
  ...
  const modules: ModuleInfo[] = names.map(name => ({
  ...
  const environmentVariableValues = new Map<string, Map<string, Set<string>>>();
  for (const [key, value] of Object.entries(allVariables)) {
    ...
    if (environment) {
      ...
      if (!HIDDEN_ENVIRONMENT_VARIABLES.has(variableName.toLowerCase())) {
        const values = environmentVariableValues.get(environment.name) || new Map<string, Set<string>>();
        const distinctValues = values.get(variableName) || new Set<string>();
  ...
  for (const environment of environments) {
    ...
    for (const [name, distinctValues] of values) {
      if (distinctValues.size === 1) environment.variables[name] = [...distinctValues][0]!;
    }
  }
