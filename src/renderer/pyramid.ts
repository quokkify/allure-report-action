...
import { AggregatedResults, LayerSummary, PYRAMID_ADVISORY } from '../allure/model.js';
 ...
 */
export function evaluatePyramidQualityGates(metrics: PyramidMetrics): QualityGates {
 ...
 */
export function formatQualityGatesMarkdownSection(
  gates: QualityGates,
  ...
  if (metrics.pyramidTotal === 0) {
    ...
    if (metrics.otherEpicTotal > 0) {
      lines.push(
        `**Note:** ${metrics.otherEpicTotal} test(s) use an unknown or unsupported \`epic\` — they do not count toward Σ pyramid layers until labels are fixed.`
      );
      lines.push('');
    }
    return lines.join('\n');
  ...
  lines.push(
    `| PYRAMID_UNIT_SHARE_LOW | ${warnIds.has('PYRAMID_UNIT_SHARE_LOW') ? '⚠️ warning' : '✓ ok'} | unit ≥ ${(100 * PYRAMID_ADVISORY.unitShareMin).toFixed(0)}% of Σ layers (actual ${(100 * metrics.unitShare).toFixed(1)}%) |`
  );
  lines.push(
    `| PYRAMID_E2E_SHARE_HIGH | ${warnIds.has('PYRAMID_E2E_SHARE_HIGH') ? '⚠️ warning' : '✓ ok'} | UI/E2E ≤ ${(100 * PYRAMID_ADVISORY.e2eShareMax).toFixed(0)}% of Σ layers (actual ${(100 * metrics.e2eShare).toFixed(1)}%) |`
  );
  lines.push(
    `| PYRAMID_UNKNOWN_EPIC | ${warnIds.has('PYRAMID_UNKNOWN_EPIC') ? '⚠️ warning' : '✓ ok'} | no epic assigned: ${metrics.otherEpicTotal} |`
  );
  lines.push('');
 ...
 */
function formatCountScaledPyramidDiagram(layers: LayerSummary[]): string[] {
  const byId = Object.fromEntries(layers.map(L => [L.id, L]));
  const rows = ['ui_e2e', 'api', 'unit']
    .map(id => byId[id])
    .filter((layer): layer is LayerSummary => layer !== undefined);
 ...
 */
export function writeQualityGatesJson(
  gates: QualityGates,
  metrics: PyramidMetrics,
  outputPath: string
): void {
 ...
 */
function githubWorkflowEscape(s: string): string {
  return String(s).replace(/%/g, '%25').replace(/\r/g, '%0D').replace(/\n/g, '%0A');
}

export function emitGithubWarning(title: string, message: string): void {
