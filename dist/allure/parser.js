 ...
 */
export function listResultFiles(resultsDir) {
 ...
 */
export function getLabelValue(labels, name) {
 ...
 */
export function getEpicForResult(result) {
    const rawEpic = getLabelValue(result.labels, 'epic');
    if (rawEpic &&
        ['unit', 'api', 'ui', 'end-to-end'].includes(rawEpic)) {
        return rawEpic;
    }
