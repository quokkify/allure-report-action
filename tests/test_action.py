...
"""
        ...
        with tempfile.TemporaryDirectory(prefix="allure-report-action-") as tmp:
            ...
            result = subprocess.run(
                [
                    ...
                    marker,
                    "--pages-url",
                    "https://example.invalid/report?existing=1",
                    "--source-run-id",
                    "32841209876",
                    "--action-version",
                    "0.1.3",
            ...
            self.assertIn("3 / 3 tests passed · 100% pass rate", body)
            self.assertIn(
                "| 3 | 3 | 0 | 0 | 0 | [View report ↗](https://example.invalid/report?existing=1&run=32841209876) |",
                body,
            )
            self.assertIn("| Unit | 1 | 1 | 0 | 0 | 0 |", body)
            self.assertIn("| E2E | 1 | 1 | 0 | 0 | 0 |", body)
            self.assertIn("| No epic assigned | 1 | 1 | 0 | 0 | 0 |", body)
            self.assertIn(
                '<sub>Generated by <a href="https://github.com/quokkify/allure-report-action">quokkify/allure-report-action</a> · '
    ...
    def test_pr_summary_statuses_failure_zero_and_unknown_results(self) -> None:
        cases = [
            ("failed", {"failed": 1, "broken": 0, "passed": 1, "skipped": 0, "unknown": 0}, "## ❌ Allure Report — failures detected", "1 failed"),
            ("empty", {"failed": 0, "broken": 0, "passed": 0, "skipped": 0, "unknown": 0}, "## ⚪ Allure Report — no tests", "No tests found · no pass rate"),
            ("unknown", {"failed": 0, "broken": 0, "passed": 1, "skipped": 0, "unknown": 1}, "Unknown", "1 unknown"),
        ]
        for name, values, heading, detail in cases:
            with self.subTest(name=name), tempfile.TemporaryDirectory(prefix="allure-report-action-status-") as tmp:
                root = Path(tmp)
                results = root / "results"
                widgets = root / "report/widgets"
                results.mkdir()
                ...
                if total:
                    for index, status in enumerate(("passed", "failed", "unknown")[:total]):
                        (results / f"{index}-result.json").write_text(json.dumps({"status": status}))
                (widgets / "summary.json").write_text(json.dumps({"statistic": {"total": total, **values}}))
                comment = root / "comment.md"
                result = self.run_cli("pr-body", "--results", str(results), "--report", str(root / "report"), "--output", str(comment), "--comment-marker", "<!-- marker -->")
                self.assertEqual(result.returncode, 0, result.stderr)
                body = comment.read_text()
